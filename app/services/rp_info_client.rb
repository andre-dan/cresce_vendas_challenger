...
class RpInfoClient
  ...
  def initialize(user:, store:)
    ...
    @circuit_breaker = CircuitBreaker::CircuitHandler.new(
      failure_threshold: 5, # Quantas falhas consecutivas para abrir o circuito
      recovery_timeout: 10, # Tempo em segundos para o circuito ficar em 'half-open'
      # exceptions: [Faraday::TimeoutError, Faraday::ConnectionFailed, StandardError], # Opcional: Quais exceptions devem abrir o circuito
  ...
  def fetch_stocks
    begin
      ...
      url = "/v1.0/produtounidade/unidade/#{@cnpj}/estoquedisponivel?codigos=#{formatted_codes}" # URL correta para estoque
