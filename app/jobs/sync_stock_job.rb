...
class SyncStockJob
  ...
  def process_items(failed_enqueues) # Renamed failed_items to failed_enqueues
    User.all.each do |user|
      Store.all.each do |store|
        ...
        Rails.logger.info("Enfileirando jobs de estoque para #{stocks.count} produtos para User ID: #{user.id}, Store ID: #{store.id}...") # More descriptive log
        stocks.each_with_index do |stock_info, index|
          Rails.logger.info("Enfileirando job de estoque #{index + 1}/#{stocks.count} (Código: #{stock_info[:codigo]}) para User ID: #{user.id}, Store ID: #{store.id}") # More descriptive log
          stock_info_strings = stock_info.deep_stringify_keys
          begin
            CronStockJob.perform_async(stock_info_strings)
          rescue => e
            failed_enqueues << stock_info[:codigo] || "Unknown Product Code" # Capture product code if available
            Rails.logger.error("Erro ao enfileirar CronStockJob para produto (Código: #{stock_info[:codigo] || 'desconhecido'}): #{e.message}")
          end
        end
      end
    end
    Rails.logger.info("Enfileiramento de jobs para estoque concluído.") # More specific log message
  end
end
