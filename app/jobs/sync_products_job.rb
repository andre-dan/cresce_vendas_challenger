...
class SyncProductsJob
  ...
  def process_items(failed_enqueues) # Renamed failed_items to failed_enqueues
    User.all.each do |user|
      ...
      client = RpInfoClient.new(user: user, store: nil) # Assuming store: nil is intentional for products
      ...
      products.each_with_index do |product_data, index|
        ...
        begin
          ...
          failed_enqueues << product_data[:codigo] || "Unknown Product Code" # Capture product code if available
          Rails.logger.error("Erro ao enfileirar CronProductJob para produto (Código: #{product_data[:codigo] || 'desconhecido'}): #{e.message}")
        end
      end
    end
    Rails.logger.info("Enfileiramento de jobs para produtos concluído.") # More specific log message
  end
end
